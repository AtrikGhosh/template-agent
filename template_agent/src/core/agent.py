...
async def get_template_agent(
    ...
    try:
        if user_id:
            preference_namespace: tuple[str, ...] = ("preferences", user_id)
            memory_namespace: tuple[str, ...] = ("memory", user_id)
        ...
        logger.info(
            "Successfully created manage preference and contextual memory tools"
        )
    except Exception as e:
        logger.error(
            f"Error creating manage preference and contextual memory tools: {e}"
        )
        memory_tools = []
    ...
    if not enable_checkpointing:
        ...
        memories = await get_contextual_memories(store, memory_namespace, message or "")
        ...
        async with (
            AsyncPostgresSaver.from_conn_string(settings.database_uri) as checkpoint,
            AsyncPostgresStore.from_conn_string(
                settings.database_uri, index=get_embedding_config()
            ) as store,
        ):
            ...
            memories = await get_contextual_memories(
                store, memory_namespace, message or ""
            )
            prompt = get_system_prompt() + preferences + memories
