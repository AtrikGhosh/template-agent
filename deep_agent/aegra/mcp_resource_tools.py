...
_CHARS_PER_TOKEN = 4
_DEFAULT_TOKEN_LIMIT = 20_000


def _get_max_chars() -> int:
    """Return the char budget matching FilesystemMiddleware's eviction threshold."""
    try:
        from deep_agent.src.agent.config import agent_config

        limit = (
            agent_config.get_filesystem_config().settings.tool_token_limit_before_evict
        )
    except (ImportError, AttributeError):
        limit = _DEFAULT_TOKEN_LIMIT
    return _CHARS_PER_TOKEN * limit
